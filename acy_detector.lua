...
--      find Collect inside Dialogue descendants → click Collect
--
--  v14 fix: Collect button is INSIDE EatFruitBecky.Dialogue (not
--           direct child). Previous versions searched wrong location.
--           Also added NoClick BoolValue bypass.
-- ═══════════════════════════════════════════════════════════════════
...
-- Character cache (rebuilt ≤every 2s — prevents isPlayerPart lag)
local charCache     = {}
...
local function refreshCharCache()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Character then charCache[p.Character] = true end
    end
end

local function isPlayerPart(obj)
    for char in pairs(charCache) do
        if obj == char or obj:IsDescendantOf(char) then return true end
    end
    return false
end
        ...
        pcall(function()
            request({
                ...
                Body    = HttpService:JSONEncode({ embeds = {{
                    ...
                    color       = clr,
                    footer      = { text = "King Legacy Boss Detector v14 | Sea 2" },
                }}})
            })
        end)
    end)
...
local function isPopupOpen()
    ...
    local ok, en = pcall(function() return eatGui.Enabled end)
...
local function findCollectButton()
    ...
    if not isPopupOpen() then return nil, "popup not visible" end

    pcall(function()
        local noClick = eatGui:FindFirstChild("NoClick")
        if noClick and noClick:IsA("BoolValue") and noClick.Value == true then
            noClick.Value = false
        end
    end)
...
local function triggerPopup(fruitTool)
    ...
    pcall(function()
        if VIM and fruitTool:FindFirstChild("Handle") then
            local sp, onSc = workspace.CurrentCamera:WorldToScreenPoint(fruitTool.Handle.Position)
            if onSc then
...
local function autoStoreFruit()
    ...
    for _, fruitTool in ipairs(fruits) do
        ...
        if not popupOk then
            ...
            local collectBtn = waitForCollectButton(4)
            if collectBtn then
                ...
                pcall(function() setStatusGUI("Store: Collect btn missing " .. fruitName, COL.red, 3) end)
                storeFailed = storeFailed + 1
            end
        end
        task.wait(0.5)
...
setStatusGUI("✅ v14 ready!", COL.green, 3)
print("[BossDetector v14] Active. Tap 'BD' to open menu.")
